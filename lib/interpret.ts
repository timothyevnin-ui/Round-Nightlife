...
export function interpretPrompt(text: string, venues: VenueForMatch[] = []) {
  ...
  return (
    ...
    (places
      ? `Places ROUND knows (slug (name, neighborhood)). If the sentence names one of them, even misspelled or shortened, set "venue" to its slug and "neighborhood" to its neighborhood; set "near": true only if they want places around it ("near", "by", "I'm at") rather than that place itself. Names that are just common words ("Local", "Diner") only count when clearly used as a place name. A word that merely appears inside a name never counts: "the West Village" or "the village" is a neighborhood, not Village Tavern; "a house party" is not Grove House. Set "venue" only when they mean that place.\n${places}\n`
      : "") +
