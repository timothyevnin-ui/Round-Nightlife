...
export type Settings = { verifiedOnly: boolean };
/**
 ...
 */
export const DEFAULT_SETTINGS: Settings = { verifiedOnly: false };
...
export async function getSettings(): Promise<Settings> {
  ...
  try {
    ...
    for (const r of rows) if (r.key === "verified_only" && typeof r.value === "boolean") out.verifiedOnly = r.value;
    return out;
  } catch {
...
export async function setSetting(key: "verified_only", value: boolean): Promise<void> {
