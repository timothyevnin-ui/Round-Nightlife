...
import type { DateStage } from "@/lib/types";
...
export default async function ResultsPage(props: PageProps<"/results">) {
  ...
  if (mode === "night") {
    ...
    const summary = [neighborhoodName(n), group === 11 ? "11+" : `${group} of you`, formatHour(hour, true), ...wantChips];
  ...
  const payload: PlanPayload = {
    ...
    n,
    t: hour,
    s: plans.map((p) => ({ restaurant: p.restaurant?.slug, bar: p.bar.slug, dinnerAt: p.dinnerAt, drinksAt: p.drinksAt, walk: p.walkMinutes })),
  };
  const code = encodePlan(payload);
  ...
  return <ResultsView mode="date" summary={summary} editHref="/plan/date" code={code} date={plans.map((p, i) => ({ ...p, shareCode: perCard[i] }))} />;
}
