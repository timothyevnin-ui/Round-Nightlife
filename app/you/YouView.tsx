...
const NightMap = dynamic(() => import("@/components/NightMap").then((m) => m.NightMap), {
  ssr: false,
  loading: () => (
    <div className="rounded-[24px] border" style={{ height: 300, borderColor: "var(--hairline)", background: "#0e1016" }} />
  ),
});
    ...
    <main className="screen screen-with-tabs mx-auto w-full max-w-md">
      ...
      <section className="relative">
        ...
        <div className="pointer-events-none absolute left-4 top-4 flex gap-3 text-[11px]" style={{ color: "var(--chalk-70)" }}>
          ...
          <Legend color="rgba(242,240,234,0.35)" label="On ROUND" small />
        </div>
        <AnimatePresence>
          {selected && (
            <motion.div
            ...
            >
              <Link href={`/v/${selected.slug}`} className="pressable card flex items-center gap-3 p-3" style={{ background: "rgba(18,20,26,0.92)", backdropFilter: "blur(12px)" }}>
      ...
      <section className="mt-6 grid grid-cols-2 gap-3">
        ...
        <Link
          ...
          style={{ background: state.quizDone ? "var(--surface)" : "linear-gradient(160deg, #1a2fb8, #2b4dff)", border: "1px solid var(--hairline)" }}
        >
          <QuizIcon />
          <div>
            <p className="text-[15px] font-medium">{state.quizDone ? "Retake the taste quiz" : "30-second taste quiz"}</p>
            <p className="mt-0.5 text-[12px] leading-snug" style={{ color: state.quizDone ? "var(--chalk-55)" : "rgba(242,240,234,0.75)" }}>
...
function Legend({ color, label, ring, small }: { color: string; label: string; ring?: boolean; small?: boolean }) {
  return (
    <span className="flex items-center gap-1.5 rounded-full px-2 py-1" style={{ background: "rgba(11,12,16,0.6)", backdropFilter: "blur(8px)" }}>
      <span
...
function ScreenshotIcon() {
  return (
    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" aria-hidden>
      <rect x="4" y="3" width="18" height="20" rx="4" stroke="#F2F0EA" strokeWidth="1.6" />
      <path d="M8 16l3.5-3.5 3 3 2-2L19 17" stroke="#4C6FFF" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="16.5" cy="9" r="1.5" fill="#4C6FFF" />
    </svg>
  );
}
...
function QuizIcon() {
  return (
    <svg width="26" height="26" viewBox="0 0 26 26" fill="none" aria-hidden>
      <rect x="6" y="4" width="14" height="18" rx="3.5" stroke="#F2F0EA" strokeWidth="1.6" transform="rotate(-8 13 13)" />
      <rect x="9" y="6" width="14" height="18" rx="3.5" stroke="#F2F0EA" strokeWidth="1.6" opacity="0.5" transform="rotate(6 16 15)" />
    </svg>
  );
}
