          ...
          <li key={v.slug}>
            <Link href={`/admin/v/${v.slug}`} className="pressable flex items-center gap-3 py-3" style={{ borderColor: "var(--hairline)" }}>
              ...
              <span className="flex shrink-0 items-center gap-1.5">
                {v.hot && (
                  <span className="rounded-full px-2 py-0.5 text-[10.5px] font-semibold tracking-wide uppercase" style={{ background: "var(--tomato)", color: "var(--on-photo)" }}>
                    Hot
                  </span>
                )}
                <span
                  className="rounded-full px-2 py-0.5 text-[10.5px] font-semibold tracking-wide uppercase"
                  style={v.verified ? { background: "var(--pine)", color: "var(--on-photo)" } : { background: "rgba(22,33,58,0.08)", color: "var(--ink-55)" }}
                >
