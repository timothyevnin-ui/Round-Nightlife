...
export function Sprint({ cards, verified, total, writable }: { cards: SprintCard[]; verified: number; total: number; writable: boolean }) {
  ...
  // Bars by default: restaurants come through ROUND's desk, so the deck is for the places only a visit can settle.
  const [kind, setKind] = useState<"bar" | "restaurant" | "all">("bar");
  const [jobs, setJobs] = useState<Job[]>([]);
  const [doneHere, setDoneHere] = useState({ verified: 0, passed: 0 });

  const ofKind = useMemo(() => (kind === "all" ? deck : deck.filter((c) => c.kind === kind)), [deck, kind]);
  const list = useMemo(() => (hood === "all" ? ofKind : ofKind.filter((c) => c.neighborhood === hood)), [ofKind, hood]);
  const counts = useMemo(() => {
    const m = new Map<string, number>();
    for (const c of ofKind) m.set(c.neighborhood, (m.get(c.neighborhood) ?? 0) + 1);
    return m;
  }, [ofKind]);
  const kindCounts = useMemo(() => ({ bar: deck.filter((c) => c.kind === "bar").length, restaurant: deck.filter((c) => c.kind === "restaurant").length }), [deck]);
    ...
    <main className="screen pb-16 pt-6" data-sprint>
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <span className="flex flex-wrap gap-2">
          <select value={kind} onChange={(e) => setKind(e.target.value as typeof kind)} className="h-11 rounded-full border px-3 text-[13.5px]" style={inputStyle} aria-label="Kind">
            <option value="bar">Bars ({kindCounts.bar})</option>
            <option value="restaurant">Restaurants ({kindCounts.restaurant})</option>
            <option value="all">Both ({deck.length})</option>
          </select>
        <select value={hood} onChange={(e) => setHood(e.target.value)} className="h-11 rounded-full border px-3 text-[13.5px]" style={inputStyle} aria-label="Neighborhood">
          <option value="all">All neighborhoods ({ofKind.length})</option>
      ...
      {current ? (
        ...
        <div className="card mt-6 px-5 py-8 text-center" data-sprint-empty>
          <p className="serif" style={{ fontSize: 26, lineHeight: 1.1 }}>
            {deck.length === 0 ? "Every place is verified or passed on." : ofKind.length === 0 ? `Every ${kind === "bar" ? "bar" : "restaurant"} is verified or passed on.` : `Nothing left in ${neighborhoodName(hood as NeighborhoodId)}.`}
          </p>
          <p className="mt-2 text-[13.5px]" style={{ color: "var(--ink-55)" }}>
            {deck.length === 0 ? "Add a place and it lands here until you've been." : ofKind.length === 0 ? "Switch the kind above to see the rest." : "Pick another neighborhood above, or walk all of them."}
          </p>
          <div className="mt-4 flex justify-center gap-2">
            {deck.length > 0 && ofKind.length > 0 && (
              <button onClick={() => setHood("all")} className="pressable btn-pine h-11 px-5 text-[14px]">
                All neighborhoods
              </button>
            )}
            {deck.length > 0 && ofKind.length === 0 && (
              <button onClick={() => setKind("all")} className="pressable btn-pine h-11 px-5 text-[14px]">
                Show both
              </button>
            )}
