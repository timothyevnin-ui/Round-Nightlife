...
| **Spots** | the fourth tab, `/spots` | Every place ROUND stands behind in one scroll: the take and ROUND's score up front, then Want to go, Been, Rate and **Disagree** on every row. Search by name, tag or cuisine; a neighborhood; bars or restaurants; best first or A to Z. Forty at a time, the rest a tap away. |
| **Disagree with our take** | Spots tab → Disagree; Studio → Disagreements | A reader says what we got wrong about a place, in their words (with a chip for what it's about: the food, the crowd, the price, the noise, the line, hours, the vibe). It lands in Studio → **Disagreements** (the dashboard shows how many are waiting). **They're right: learn it** sends their words through Read my words in reader mode: the algorithm's attributes, tags, food, price, room, walk-in odds, hours and day deal fill in from them and the exchange is logged in the place's notes, dated; the take itself never changes (it's yours to rewrite). **Not quite** files it. Signed in, a disagreement carries the account and the name; otherwise it's just "Someone". Ten a day per person, a honeypot for bots, nothing public. Needs the V19 `schema.sql` (the `disputes` table). |
