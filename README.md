...
| Surface | Route | Notes |
| --- | --- | --- |
| **When** | Home, top | "It's Wednesday · 3:40pm" reads the phone's clock. Tap it to plan ahead: pick the day (Today, Tomorrow, Sat…) and slide the time from 11am to 3am, and every flow, Just say it and Near me start from that until you tap Now (it forgets itself after six hours). Nothing on the page changes with the time except the little sun or moon on the chip. But a daytime hour (by the clock, or because the person set it to noon) is the cue: the questions change (sun or shade, lazy or proper, deal hunting, a game, food), the engine leans on each place's "good in daylight" score, and Claude is told it's daytime. Night out is Night out; it just picks day places. |
