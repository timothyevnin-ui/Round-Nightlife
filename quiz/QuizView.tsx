  ...
  return (
    ...
    >
      <Photo venue={venue} rounded="rounded-[28px]" className="h-full w-full">
        ...
        >
          <span className="rounded-full px-3 py-1.5" style={{ background: "var(--tomato)", color: "var(--on-photo)" }}>WANT TO GO</span>
        </motion.span>
        <motion.span style={{ opacity: passOpacity }} className="absolute right-5 top-5 text-[12px] font-semibold tracking-[0.12em]">
          <span className="rounded-full px-3 py-1.5" style={{ background: "rgba(22,33,58,0.78)", color: "var(--on-photo)" }}>PASS</span>
        </motion.span>
        <div className="absolute inset-x-0 bottom-0 p-6">
          ...
          <p className="mt-2 text-[14px] leading-snug" style={{ color: "var(--on-photo-80)" }}>
            {venue.take}
          </p>
        </div>
      </Photo>
  ...
  const style =
    accent === "cobalt"
      ...
      : accent === "chalk"
        ...
        : { background: "var(--ink-6)", color: "var(--ink)", borderColor: "var(--hairline)" };
  return (
